...
class ScriptLoader {
  ...
  processExpression(expression, cb = null) {
    ...
    try {
      ...
      if (!/^(.*)\*\.js$/.test(expression)) {
        throw 'only *.js is allowed pattern'
      }

    } catch (e) {
      throw new toweran.InvalidArgumentException(`Wrong expression given '${expression}', expected glob pattern to *.js files or existing dir!`)
    }
