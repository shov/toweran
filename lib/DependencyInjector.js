 ...
 * TODO: refactor, very dirty and not clear solution
 */
class DependencyInjector {
  ...
  _dotNotationClassRegistration(pattern, filePath, base = null) {
    ...
    const begin = this._catPattern(pattern)
    const beginPos = pattern.lastIndexOf(begin)

    base = base ? base + '.' : ''

    const key = (base + filePath.slice(beginPos))
      .replace(/^(.*)\.js$/, '$1')
      .replace(/\//g, '.')
  ...
  _parseConfigSet(set, setIndex = '-') {
    ...
    //Validate paths
    ;[
      {bunch: 'include', paths: pathData.include},
      {bunch: 'exclude', paths: pathData.exclude},
    ].forEach(bunch => {
      bunch.paths.forEach(pattern => {
        let message
        switch (true) {
          case /(\.\/|~\/)+/.test(pattern):
            message = message || `Pattern must not contain relative paths!`
          case !(new RegExp(`^(${toweran.APP_PATH}|${toweran.FRAMEWORK_PATH})\/[^*/]+`)).test(pattern):
            message = message || `Pattern must start from the application root dir or the framework path, but mustn't point to a root itself!`

            throw new toweran.InvalidArgumentException(message)
        }
      })
    })
   ...
   * @private
   */
  _getCandidatesCandidates(pathData, strategy, base = null, strict) {
  ...
  _catPattern(pattern) {
    ...
    // /a/b/c/app/**/*.js -> app/
    // /a/b/c/app/domain/*.js -> domain/
    // /a/b/c/app/http -> http
    // /a/b/c/app/mix*/*.js -> mix
    // / -> null, will throw an exception

    const patternParts = pattern.match(/^(.*)\/([^/*]+\/?)(.*)$/)
    if (!check.array(patternParts) || !patternParts[2]) {
