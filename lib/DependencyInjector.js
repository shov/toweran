...
class DependencyInjector {
   ...
   */
  constructor(logger, config, container, scriptLoader, annotationInspector) {
    /**
     ...
     * @type {symbol}
     */
    this.DEFAULT_STRATEGY = CONST.DI.DOT_NOTATION
  }

  /**
   * @typedef {{
   *   path: string|{include: undefined|string|string[], exclude: undefined|string|string[]},
   ...
   * Any path expression can be directory path or glob pattern,
   * Basically it's DIConfigSet, here are examples
   * {{di: [
   *   {
  ...
  init() {
    ...
    this.registerByConfig(this._config.app.di())
  }

  /**
   * Register everything that possible by given config
   * @see init description of a set to get the config idea
   * @param {DIConfigSet|DIConfigSet[]|[]} diConfig
   */
  registerByConfig(diConfig) {
    if (check.object(diConfig)) {
      diConfig = [diConfig]
    }

    must.be.array(diConfig)

    //Parse the config
    let candidates = diConfig.reduce((candidates, set, setIndex) => {
   ...
   * @param {*[]} args
   */
  registerClass(pattern, filePath, strategy = CONST.DI.DOT_NOTATION, ...args) {
    must.be.notEmptyString(pattern)
      .and.be.notEmptyString(filePath)

    if (!this._isStrategyValid(strategy)) {
      throw new InvalidArgumentException(`Not known strategy, cant register a class! (pattern ${pattern}, file ${filePath})`)
    }

    switch (strategy) {
      case CONST.DI.DOT_NOTATION:
        try {
   ...
   * @param {*[]} args
   */
  registerClassStrict(pattern, filePath, strategy = CONST.DI.DOT_NOTATION, ...args) {
    must.be.notEmptyString(pattern)
      .and.be.notEmptyString(filePath)
  ...
  _dotNotationClassRegistration(pattern, filePath, base = null) {
    ...
    //TODO: add a possibility to register factories and singletons by annotations @url https://trello.com/c/fKYYgijT/37-di-annotationns-args-functions
  ...
  _parseConfigSet(set, setIndex = '-') {
    ...
    if (!check.nonEmptyString(set.path) && !check.object(set.path)) {
      throw new InvalidArgumentException(`DI path must be nonempty string or object! Set index: ${setIndex}`)
    }
    ...
    if (!pathData.include || pathData.include.length < 1) {
      throw new InvalidArgumentException(`DI path set must includes at least one path! Set index: ${setIndex}`)
    }
    ...
    if (!this._isStrategyValid(strategy)) {
      throw new InvalidArgumentException(`DI path set strategy is not known! Set index: ${setIndex}`)
    }
 ...
 * @return {boolean}
 */
function doesPatternContainRelativePath(pattern) {
  return /(\.\/|~\/)+/.test(pattern)
}
 ...
 * @return {boolean}
 */
function doesPatternStartFromAppPath(pattern) {
  const escapedAppPath = escapeStringRegexp(APP_PATH)
 ...
 * @return {boolean}
 */
function doesPatternStartFromFrameworkPath(pattern) {
